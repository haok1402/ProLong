...
class Worker:
    ...
    def __init__(self):
        ...
        try:
            # To remain consistent with the transformers version used in this codebase, we set
            # use_fast=False here. The fast tokenizer for Qwen3 gets added in 4.51.0, while the
            # current codebase uses 4.44.2 (see requirements.txt).
            Worker.dst_tokenizer = AutoTokenizer.from_pretrained("Qwen/Qwen3-0.6B", local_files_only=True, use_fast=False)
        except Exception:
            Worker.dst_tokenizer = AutoTokenizer.from_pretrained("Qwen/Qwen3-0.6B", use_fast=False)
